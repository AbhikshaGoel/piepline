"""
setup.py - First-time setup utility.
Run once after installing requirements:  python setup.py
Creates dummy image, initializes DB, validates config.
"""
import sys
from pathlib import Path

ROOT = Path(__file__).parent


def create_dummy_image():
    """Generate a branded placeholder image for posts."""
    try:
        from PIL import Image, ImageDraw, ImageFont
        import config

        img  = Image.new("RGB", (1080, 1080), color=(15, 20, 40))
        draw = ImageDraw.Draw(img)

        # Gradient-ish overlay
        for y in range(1080):
            alpha = int(y / 1080 * 60)
            draw.line([(0, y), (1080, y)], fill=(0, 100 + alpha, 200))

        # Text
        name  = config.INSTANCE_DISPLAY
        lines = [name, "📰 Latest News", "Stay Informed"]

        y_start = 380
        for i, line in enumerate(lines):
            # Approximate centering
            x = 1080 // 2 - len(line) * 14
            y = y_start + i * 100
            draw.text((x, y), line, fill=(255, 255, 255))

        out = config.DUMMY_IMAGE
        img.save(out, "JPEG", quality=90)
        print(f"✅ Dummy image created: {out}")
    except ImportError:
        # Pillow not available: create a minimal JPEG placeholder
        import config
        # Minimal valid JPEG (1x1 white pixel)
        jpeg_bytes = bytes([
            
        ])
        config.DUMMY_IMAGE.write_bytes(jpeg_bytes)
        print(f"✅ Minimal dummy image created: {config.DUMMY_IMAGE}")
    except Exception as e:
        print(f"⚠️  Could not create dummy image: {e}")


def check_imports():
    """Check which optional packages are available."""
    checks = [
        ("feedparser",           "RSS parsing"),
        ("sentence_transformers","Local AI embeddings"),
        ("chromadb",             "Vector DB (same-day dedup)"),
        ("requests_oauthlib",    "Twitter/X posting"),
        ("PIL",                  "Image generation"),
        ("apscheduler",          "APScheduler (optional)"),
        ("googleapiclient",      "YouTube (optional)"),
    ]
    print("\n📦 Package check:")
    for pkg, desc in checks:
        try:
            __import__(pkg)
            print(f"  ✅ {pkg:<28} {desc}")
        except ImportError:
            marker = "⚠️ " if "optional" in desc else "❌"
            print(f"  {marker} {pkg:<28} {desc} — NOT INSTALLED")


def main():
    print("🔧 News AI Pipeline — Setup\n")

    # Create directories
    ROOT.joinpath("data").mkdir(exist_ok=True)
    ROOT.joinpath("logs").mkdir(exist_ok=True)
    ROOT.joinpath("media").mkdir(exist_ok=True)
    print("✅ Directories: data/ logs/ media/")

    # Check .env
    env_file = ROOT / ".env"
    if not env_file.exists():
        import shutil
        shutil.copy(ROOT / ".env.example", env_file)
        print("✅ .env created from .env.example — EDIT IT before running")
    else:
        print("✅ .env exists")

    # Import check
    check_imports()

    # Create dummy image
    create_dummy_image()

    # Init DB
    try:
        import db
        db.init_db()
        print("✅ Database initialized")
    except Exception as e:
        print(f"❌ DB init failed: {e}")

    # Config validation
    try:
        import config
        problems = config.validate()
        if problems:
            print(f"\n⚠️  Config issues (edit .env to fix):")
            for p in problems:
                print(f"   • {p}")
        else:
            print("✅ Config looks good")
    except Exception as e:
        print(f"⚠️  Config check failed: {e}")

    print("\n🚀 Setup complete!")
    print("   Run test:  python main.py --test")
    print("   Run live:  python main.py --live")
    print("   Schedule:  python scheduler.py\n")


if __name__ == "__main__":
    main()
